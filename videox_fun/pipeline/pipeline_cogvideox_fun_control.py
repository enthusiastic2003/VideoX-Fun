...
class CogVideoXFunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        source_depth_video: Optional[torch.FloatTensor] = None,
        ...
        Args:
            ...
            latents (`torch.FloatTensor`, *optional*):
                Pre-encoded latents from the source/original video. Used for the Source Branch to preserve appearance
                characteristics. The latents will be noised based on the `strength` parameter for appearance-motion
                tradeoff. If not provided, only the Control Branch (pure noise) will be used.
            ...
            source_depth_video (`torch.FloatTensor`, *optional*):
                Depth video extracted from the source/original video for dual-branch denoising pipeline.
                Enables appearance-preserving video generation through parallel denoising branches:
                
                **Control Branch**: Initialized with pure noise, guided by `control_video` and `prompt`.
                    Generates motion/structure based on the control video and target prompt alone.
                
                **Source Branch**: Initialized with noised `latents` (encoded source video), guided by
                    `source_depth_video` and `source_prompt`. Preserves appearance characteristics from
                    the original video while adapting to the target prompt.
                
                The `strength` parameter (0-1) interpolates between these branches:
                - strength=1.0: Full denoising with appearance preservation (Source Branch dominant)
                - strength=0.0: Pure motion control without appearance modification (Control Branch dominant)
                
                Should be aligned with `control_video` in frame count, height, and width.
                The depth video should be a single-channel video where pixel values represent depth.
            
            source_prompt (`str`, *optional*):
                Text prompt describing source/original video characteristics for appearance-preserving generation.
                Used to guide the Source Branch denoising to maintain appearance features from the original video.
                If not provided but `source_depth_video` is provided, falls back to the main `prompt` instead.
        ...
        # ============================================================================
        # 5. Prepare latents (Control Branch)
        # [MODIFIED] Always generate pure noise latents for control branch
        # ============================================================================
        latent_channels = self.vae.config.latent_channels
        
        # Always generate pure noise latents for control branch denoising
        latents = self.prepare_latents(
            ...
            device,
            generator,
            None,  # Force random sampling
        )
        print("Latents Shape after preparation (Control Branch - Pure Noise): ", latents.shape)
        # ============================================================================
        if comfyui_progressbar:
            pbar.update(1)
        ...
        # Encode source prompt if provided
        if source_prompt is not None:
            ...
            # Apply classifier-free guidance concatenation to match main prompt format
            if do_classifier_free_guidance:
                source_prompt_embeds = torch.cat([source_negative_prompt_embeds, source_prompt_embeds], dim=0)
            print("Source Prompt Embeddings Shape: ", source_prompt_embeds.shape)
        
        # Prepare source latents from input clean source latents
        if source_latents is not None:
            # Move to device and convert dtype
            source_latents_clean = source_latents.to(device=device, dtype=prompt_embeds.dtype)
        ...
        if source_depth_video is not None:
            # 1. Spatial preprocessing: resize/crop to match target dimensions
            source_depth_video_length = source_depth_video.shape[2]
            source_depth_video = self.image_processor.preprocess(
                rearrange(source_depth_video, "b c f h w -> (b f) c h w"), 
                height=height, width=width
            )
            source_depth_video = source_depth_video.to(dtype=torch.float32)
            source_depth_video = rearrange(source_depth_video, "(b f) c h w -> b c f h w", f=source_depth_video_length)
            
            # 2. Temporal frame alignment (use same video_length as control_video for consistency)
            if source_depth_video_length > video_length:
                source_depth_video = source_depth_video[:, :, :video_length]
            
            # 3. VAE encode to latent space
            source_depth_video_latents = self.prepare_control_latents(
                None,
                source_depth_video,
                batch_size,
                height,
            ...
            # 4. Classifier-free guidance duplication
            source_depth_video_latents_input = (
                torch.cat([source_depth_video_latents] * 2) 
                if do_classifier_free_guidance 
                else source_depth_video_latents
            )
            print("Source Depth Video Shape after encoding: ", source_depth_video_latents_input.shape)
